...
#define LOG_TAG "audio_policy_tegra"
//#define LOG_NDEBUG 0
...
struct tegra_ap_module {
    struct audio_policy_module module;
};

struct tegra_ap_device {
    struct audio_policy_device device;
};

struct tegra_audio_policy {
...
static int create_tegra_ap(const struct audio_policy_device *device,
                             struct audio_policy_service_ops *aps_ops,
                             void *service,
                             struct audio_policy **ap)
{
    struct tegra_ap_device *dev;
    struct tegra_audio_policy *dap;
    int ret;
    ...
    *ap = NULL;
    ...
    dap = (struct tegra_audio_policy *)calloc(1, sizeof(*dap));
    if (!dap)
        return -ENOMEM;
...
static int destroy_tegra_ap(const struct audio_policy_device *ap_dev,
                              struct audio_policy *ap)
{
    free(ap);
    return 0;
...
static int tegra_ap_dev_close(hw_device_t* device)
{
    free(device);
    return 0;
}

static int harmon_ap_dev_open(const hw_module_t* module, const char* name,
                               hw_device_t** device)
{
    struct tegra_ap_device *dev;
    ...
    *device = NULL;
    ...
    dev->device.common.close = tegra_ap_dev_close;
    dev->device.create_audio_policy = create_tegra_ap;
    dev->device.destroy_audio_policy = destroy_tegra_ap;
...
static struct hw_module_methods_t tegra_ap_module_methods = {
    .open = tegra_ap_dev_open,
};

struct tegra_ap_module HAL_MODULE_INFO_SYM = {
    .module = {
        .common = {
            ...
            .name           = "Tegra audio policy HAL",
            .author         = "The Android Open Source Project",
            .methods        = &tegra_ap_module_methods,
        },
    },
};
